...
fmtDisassembly str = fmtDisassemblyFields (disassembleHex str)
--
--
fmtDisassemblyFields::[(String,[Int],Integer)]->String
fmtDisassemblyFields [] = ";"
fmtDisassemblyFields (x:xs) = fmtDisassemblyField x ++ fmtDisassemblyFields xs
--
--
--
...
getBitList  val [] = []
getBitList  val (x:xs) = [(getABit val x)] ++ getBitList val xs
--
...
getBitListVal val ls = intfrombits ( getBitList val ls)
--
...
--
fieldTrue::Integer->([Int], Val)->Bool
fieldTrue inval (a, Sval b) = True
fieldTrue  inval (a,Ival b) = (evalField inval  a) == b
--
...
--
filterMacro::Integer->[Macro]->[Macro]
filterMacro inval mls = filter (evalMacro inval) mls
...
integerfromStr x | ( head x == '#') = foldr (+) 0 (map (topowsixteenI)  ( zip (reverse ( drop 1 x)  ) ( map (16 ^ ) [0..] )) )
             | ( head x == 'B') = foldr (+) 0 (map (topowtwoI) ( zip (reverse (drop 1 x  )) ( map (2 ^ ) [0..] )) )
             | ( head x == 'O') = foldr (+) 0 (map (topoweightI)  ( zip (reverse (drop 1 x  )) ( map (8 ^ ) [0..] )) )
             | otherwise = foldr (+) 0 (map (topowI)  ( zip (reverse x  ) ( map (10 ^ ) [0..] )) )
--
--
--
...
main = print ( fmtDisassembly (reverse "D0E000CD7BF0000F280000000" ))
